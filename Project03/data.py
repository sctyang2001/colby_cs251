...
class Data:
    ...
    def read(self, filepath):
        ...
        with open(self.filepath, 'r') as file:
            ...
            for row in reader:
                temReader.append(row)

            #find out what cols are numeric variables at (temReader[1])
            if "numeric" in temReader[1]:
                for i in range(len(temReader[1])):
                    if temReader[1][i] == "numeric":
                        numCol.append(i)
            else:
            ...
            if self.headers == None:
                self.headers = []
                for j in numCol:
                    self.headers.append(temReader[0][j].strip())
            ...
            if self.data == None:
                self.data = []
                for row in temReader[2:]:
                    arrayRow = []
                    for j in numCol:
                        arrayRow.append(float(row[j]))
                    self.data.append(arrayRow)
    ...
    def __str__(self):
        ...
        string = "-------------------------------\nHeaders:\n"
        string += "  ".join([str(elem) for elem in self.headers])
        if len(self.data) >=5:
            string += "\n-------------------------------\nShowing first 5/"
            string += str(len(self.data))
            string += " rows. \n"


            for i in range(5):
                string += "   ".join([str(elem) for elem in self.data[i]])
                string += "\n"
        else:
            string += "\n-------------------------------\nShowing all rows. \n"
    ...
    def get_header_indices(self, headers):
        ...
        for i in headers:
            output.append(self.header2col[i])
    ...
    def radomize_data(self):
        np.random.shuffle(self.data)



if __name__ == '__main__':
    d1 = Data(filepath = 'data/anscombe.csv')
    print(d1)
    #print(d1.get_mappings())
    print('\n\n')
    print('all shape: ', d1.get_all_data().shape)
    print('head shape: ', d1.head().shape)
    print('tail shape: ', d1.tail().shape)
    #print(d1.get_header_indices(['sepal_width','petal_length']))