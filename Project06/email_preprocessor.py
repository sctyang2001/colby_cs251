...
def count_words(email_path='data/enron/'):
    ...
    pathes = os.listdir(email_path) 

    for i in range(len(pathes)):
        path = email_path+pathes[i]
        if os.path.isdir(path) == True:
            for root, dirs, files in os.walk(path):
                for file in files:
                    num_emails += 1
                    words = tokenize_words(open(path+'/'+file).read())
                    for word in words:
...
def find_top_words(word_freq, num_features=200):
    ...
    '''
    top_words = sorted(word_freq, key=word_freq.get, reverse=True)[:num_features]
    counts = sorted(word_freq.values(), reverse=True)[:num_features]
                ...
                for file in files:
                    ...
                    for word in words:
                        if word in top_words:
                            feat[num][top_words.index(word)] += 1
                    num += 1

    return feat, np.array(y)
...
def make_train_test_sets(features, y, test_prop=0.2, shuffle=True):
    ...
    # Your code here:
    num_train = y.size - int(y.size*test_prop)
    num_test = int(y.size*test_prop)
...
def words_count_exclusive(email_path='data/enron/'):
    '''Build a function where stop words could be excluded in the word count process.
    '''
