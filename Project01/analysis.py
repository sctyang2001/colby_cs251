...
class Analysis:
    ...
    def min(self, headers, rows=[]):
        ...
        '''
        unprocessed = self.data.select_data(headers, rows)
        return unprocessed.min(axis=0)
    ...
    def range(self, headers, rows=[]):
        ...
        '''
        unprocessed = self.data.select_data(headers, rows)
        return unprocessed.min(axis=0), unprocessed.max(axis=0)
    ...
    def mean(self, headers, rows=[]):
        ...
        '''
        unprocessed = self.data.select_data(headers, rows)
        addition = unprocessed.sum(axis=0)
        mean = addition / len(unprocessed)
        return mean
    ...
    def var(self, headers, rows=[]):
        ...
        squared = (unprocessed - mean) ** 2 # the square of all means minus numbers
        var = squared.sum(axis = 0)/(len(unprocessed)-1)
        return var
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        '''
        fig, axes = plt.subplots(nrows=len(data_vars), ncols=len(data_vars), figsize=fig_sz, sharex=True, sharey=True) # using tuple unpacking for multiple Axes, the grids have shared x-axis and y-axis
